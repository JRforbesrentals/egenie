...
<!--#include file="includes/header.inc.asp" -->
  ...
  <div class="row clearfix">
    <div class="col-12">
        <h2 class="textLeft choose"><%=(rsMain.Fields.Item("CatTitle").Value)%></h2>
...
response.write("<div class='module-box'><p>Sorry but no products match your search.  Please try again.</p></div>")
...
<article class="module-product-box">
      <h2 class="centered">
        <a href="product.asp?<%= Server.HTMLEncode(MM_keepNone) & MM_joinChar(MM_keepNone) & "prodid=" & rsMain.Fields.Item("prodid").Value %>">
              <%=(rsMain.Fields.Item("Make").Value)%> <br><span class="small"><%=(rsMain.Fields.Item("Model").Value)%> Rental</span></a>
        </a>
      </h2>
     <a href="product.asp?<%= Server.HTMLEncode(MM_keepNone) & MM_joinChar(MM_keepNone) & "prodid=" & rsMain.Fields.Item("prodid").Value %>">
   ...
   <div class="product-desc">
    ...
    <% End If %>
    <% If Not(rsMain.Fields.Item("specialprice").Value)=0 Then %>      
          <p class="small normalprice centered">Normal Price:<br /><del><%= FormatCurrency((rsMain.Fields.Item("rprice").Value), 2, -2, -2, -2) %></del></p>
          <% End If %>
        <form id="applyNowForm" method="post" action="apply.asp" class="product-buttons clearfix">
            ...
            <input type="submit" name="apply" id="apply" value="Apply Now" class="button-apply" />
          <a href="product.asp?<%= Server.HTMLEncode(MM_keepNone) & MM_joinChar(MM_keepNone) & "prodid=" & rsMain.Fields.Item("prodid").Value %>" class="button-moreinfo" />Details</a> 
        </form>
   </div>
  </article>
