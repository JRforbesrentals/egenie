  ...
  <div class="clearfix"></div>
   <div class="contentwrapper mainbody">
         <div class="row clearfix">
           <div class="col-4 col-expanded float-right">
            <div class="module-apply">
                <div class="mobile-only">
                  <h2>Apply now for this:</h2>
                   <hgroup>
                     <h3><%=(rsPricelist.Fields.Item("make").Value) & " " & (rsPricelist.Fields.Item("type").Value)%></h3>
                     <h4><%=(rsPricelist.Fields.Item("model").Value)%></h4>
                   </hgroup>
 				   <img src="../../<%=(rsPricelist.Fields.Item("imageBig").Value) %>" alt="<%=(rsPricelist.Fields.Item("make").Value)%> &nbsp;<%=(rsPricelist.Fields.Item("model").Value)%>" class="centered product-image" />
                    <% If Not(rsPricelist.Fields.Item("specialprice").Value)=0 Then %><p class="specialrate centered">Special Rate</p><p class="rentaltype producttype-">Pay weekly</p>
                        <p class="price-large"><%= FormatCurrency((rsPricelist.Fields.Item("Specialprice").Value), 2, -2, -2, -2) %></p>
                            <% Else %>
                        <p class="centered producttype-">Pay Weekly
                        <p class="price-large centered" title="Monthly <%= FormatCurrency((rsPricelist.Fields.Item("rprice").Value*52/12), 2, -2, -2, -2) %>"><%= FormatCurrency((rsPricelist.Fields.Item("rprice").Value), 2, -2, -2, -2) %><% If (rsPricelist.Fields.Item("Category").Value) = "ZC" Then %> excl VAT <% End If %></p>
                    <% End If %>
                        </p>
                </div>
                ...
                <form id="applyNowForm" method="post" action="apply.asp">
                        ...
                        <input type="hidden" name="productImage" value="<%=(rsPricelist.Fields.Item("imageBig").Value) %>">
                        <input type="hidden" name="rprice" value="<%=(rsPricelist.Fields.Item("rprice").Value) %>">
                        <input type="submit" name="apply" id="apply" value="Apply Now" class="button-primary" />
                 </form>
                  <p class="or">Or</p>
                  <p class="tel">Call Us</p>
                  <p class="tel"><a href="tel:03450702337">0345 0702337</a></p>
            </div>

            <div class="module-box no-bottom-margin">
              <h3 class="features centered">Product Features<br><span class="small oblique"><%=(rsPricelist.Fields.Item("make").Value) & " " & (rsPricelist.Fields.Item("type").Value) & " " %></span></h3>
                ...
                <p class="centered producttype-features">Normal Price:</p>
                <p class="price-features centered"><del><%= FormatCurrency((rsPricelist.Fields.Item("rprice").Value), 2, -2, -2, -2) %></del></p>
                <% End If %>
            ...
            <% If Not(rsPricelist.Fields.Item("specialprice").Value)=0 Then %><p class="specialrate centered">Special Rate</p><p class="rentaltype producttype-">Pay Weekly</p>
            <p class="price-"><%= FormatCurrency((rsPricelist.Fields.Item("Specialprice").Value), 2, -2, -2, -2) %></p>
                <% Else %><p class="centered producttype-">Pay weekly
            <p class="price- centered" title="Monthly <%= FormatCurrency((rsPricelist.Fields.Item("rprice").Value*52/12), 2, -2, -2, -2) %>"><%= FormatCurrency((rsPricelist.Fields.Item("rprice").Value), 2, -2, -2, -2) %><% If (rsPricelist.Fields.Item("Category").Value) = "ZC" Then %> excl VAT <% End If %></p>
            <% End If %>
            ...
            <p class="centered rentthis"><strong>Apply now for this:</strong></p>     
                  ...
                  <form id="applyNowForm2" method="post" action="apply.asp">
                        <input type="hidden" name="product" value="<%=(rsPricelist.Fields.Item("make").Value) & " " & (rsPricelist.Fields.Item("model").Value) & " " & (rsPricelist.Fields.Item("type").Value)%>" />
                        <input type="hidden" name="rprice" value="<%=(rsPricelist.Fields.Item("rprice").Value) %>">
                        <input type="hidden" name="productImage" value="<%=(rsPricelist.Fields.Item("imageBig").Value) %>">
                      <input type="submit" name="apply" id="apply" value="Apply Now" class="button-rentproduct" />
                  </form>


            <p class="centered"><strong>from <span class="egenie"><em>e</em>genie</span> today.</strong></p>
          ...
          <!--   
            <div class="module-product-width-wrap"> 
